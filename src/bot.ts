...
client
	...
	.on("messageCreate", async (message) => {
		if (!(await unpartial(message))) return;
		...
		if (message.channel.type == ChannelType.GuildAnnouncement) {
			...
			if (message.channel.name.includes("voice-text")) {
				message.attachments.some((attachment) => {
					...
					if (
						file != undefined &&
						(file.includes("output_log") || file.includes("Player.log"))
					) {
						message.channel
							.send({
								embeds: [
	...
	.on("voiceStateUpdate", async (oldState, newState) => {
		...
		function convert(i: number, names: string[]): string {
			return i < names.length
				? names[i]
				: `${convert(((i / names.length) | 0) - 1, names)} ${names[i % names.length]}`;
		}
		...
		function processAutoManagedCategories(vc: BaseGuildVoiceChannel | null) {
			...
			if (
				!cat ||
				!vc.parentId ||
				!(vc.parentId in tokens.autoManagedCategories) ||
				cat === catProcessed
			)
				return;
			catProcessed = cat;
			...
			const ignore =
				tokens.autoManagedCategories[vc.parentId].ignoredChannels || [];
			const channels = [...cat.children.cache.values()]
				.filter(
					(ch) =>
						ch.type === ChannelType.GuildVoice &&
						ignore.filter((ig) => ig === ch.id).length === 0,
				)
				.map((ch) => ({ channel: ch, members: ch.members.size }));
			channels.sort((c1, c2) =>
				c1.channel.name < c2.channel.name
					? -1
					: c1.channel.name > c2.channel.name
						? 1
						: 0,
				...
				if (channels.length === 1 || channelsForceRename) {
					for (; ix < channels.length; ix++) {
						if (channels[ix].channel.name !== `${prefix} ${convert(ix, names)}`)
							channels[ix].channel
								.setName(`${prefix} ${convert(ix, names)}`)
								.catch(Logger.errorPrefix("Failed to set channel name:"));
					}
				...
				for (let i = 0; i < channels.length; i++) {
					if (channels[i].members === 0) {
						if (oneFound) {
							...
							channels[i].channel
								.delete()
								.catch(Logger.errorPrefix("Failed to delete channel:"));
						} else oneFound = true;
					}
				}
			}
	...
	.on("presenceUpdate", (_, newPresence) => {
		...
		checkStreamingStatus(newPresence, true).catch(
			Logger.errorPrefix("checkStreamingStatus"),
		);
	})
	.on(
		"messageReactionAdd",
		async (reaction, user) => await handleReaction(reaction, user, true),
	)
	.on(
		"messageReactionRemove",
		async (reaction, user) => await handleReaction(reaction, user, false),
	);
...
async function handleReaction(
	...
	if (!(await unpartial(reaction)) || reaction.partial) return;

	const message = reaction.message;
	if (!(await unpartial(message)) || message.partial) return;
	...
	if (
		channel.id == "612414629179817985" ||
		channel.parentId == "612414629179817985"
	) {
		if (
			!reactionAdded ||
			reaction.emoji.name != "solved" ||
			message.pinned ||
			!message.guild.members.cache
				.get(user.id)
				?.roles.cache.has(tokens.roleIDs.maintainer)
		)
			return;
		...
		for (const [menuMessageID, emojis] of Object.entries(
			...
			for (const [emojiName, roleID] of Object.entries(emojis)) {
				if (reaction.emoji.name != emojiName) continue;
				...
				if (
					menuMessageID == "640560537205211146/640563515945385984" &&
					client.user &&
...
CronJob.from({
	...
	onTick: () => {
		...
		workshopScanner
			.run()
			.catch((error: unknown) =>
				Logger.error("Unable to run workshop scan:", error),
			);
