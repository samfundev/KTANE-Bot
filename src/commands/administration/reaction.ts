	...
	slashOptions: [
		{
			...
			description: "The user you want to toggle reacting for.",
		},
		{
			name: "duration",
			type: ApplicationCommandOptionType.String,
			description: "The duration of the reaction restriction.",
			required: false,
		},
	],
})
export default class ReactionCommand extends MixedCommand {
	async run(msg: MixedInteraction<true>, args: Args): Promise<void> {
		...
		const duration = await args
			.pick({ name: "duration", type: "duration" })
			.catch(() => null);

		if (target.roles.highest.comparePositionTo(msg.member.roles.highest) >= 0) {
			await msg.reply(
				`${target.user.username} has a equal or higher role compared to you.`,
			);
			return;
		}
		...
		if (noReaction) {
			TaskManager.removeTask(
				"removeRole",
				(task) =>
					task.roleID == tokens.roleIDs.noReaction &&
					task.memberID == target.id,
			);

			target.roles
				.remove(tokens.roleIDs.noReaction)
				.then(async () =>
					msg.reply(`${target.user.username} has been allowed to react.`),
				)
			...
			if (duration != null)
				TaskManager.addTask({
					...
					type: "removeRole",
					roleID: tokens.roleIDs.noReaction,
					memberID: target.id,
					guildID: msg.guild.id,
				});

			target.roles
				.add(tokens.roleIDs.noReaction)
				.then(async () =>
					msg.reply(
						`${target.user.username} has been prevented from reacting.`,
					),
				)
