...
@ApplyOptions<MixedOptions>({
	...
	aliases: ["mute", "m"],
	description:
		"Toggles if someone is allowed to speak in text and voice channels.",
	runIn: "GUILD_ANY",
	...
	async run(msg: MixedInteraction<true>, args: Args): Promise<void> {
		...
		if (target.roles.highest.comparePositionTo(msg.member.roles.highest) >= 0) {
			msg
				...
				.catch(Logger.errorPrefix("Failed to send message:"));
			return;
		}
		...
		if (muted) {
			target.roles
				.remove(tokens.roleIDs.voiceMuted)
				.then(async () => {
					TaskManager.removeTask(
						"removeRole",
						(task) =>
							task.roleID == tokens.roleIDs.voiceMuted &&
			...
			if (duration != null)
				TaskManager.addTask({
					...
					type: "removeRole",
					roleID: tokens.roleIDs.voiceMuted,
					memberID: target.id,
		...
		if (target.voice.channel)
			target.voice
				.setMute(!muted)
				.catch(async () => msg.reply("Unable to change server mute status."));
	}
}
