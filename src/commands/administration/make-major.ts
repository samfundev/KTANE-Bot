	...
	slashOptions: [
		{
			...
			description: "The ID of the message you want to make major.",
		},
	],
})
export default class MakeMajorCommand extends MixedCommand {
	async run(msg: MixedInteraction, args: Args): Promise<void> {
		...
		if (message.embeds.length != 1) {
			await msg.reply({
				content: "Invalid number of embeds on target message.",
				ephemeral: true,
			});
			return;
		}
		...
		if (
			targetEmbed.timestamp === null ||
			targetEmbed.title === null ||
			targetEmbed.url === null ||
		...
		sendWebhookMessage(
			...
			new WebhookClient(tokens.majorWebhook),
			{
