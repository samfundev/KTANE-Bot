...
export default class RoleCommand extends MixedCommand {
	async run(msg: MixedInteraction<true>, args: Args): Promise<void> {
		...
		const roleInf = getRole(
			role,
			tokens.roleIDs.assignable,
		...
		if (roleInf !== null) {
			...
			if (
				roleData.prereq &&
				!roleData.prereq.some((pre) => msg.member!.roles.cache.has(pre))
			) {
				await msg.reply({
					content: `You can’t self-assign the "${role.name}" role because you don’t have any of its prerequisite roles.`,
					ephemeral: true,
				});
				return;
			}
