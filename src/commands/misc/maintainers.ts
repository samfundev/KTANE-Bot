...
export default class MaintainersCommand extends MixedCommand {
	async run(message: MixedInteraction<true>, args: Args): Promise<void> {
		if (
			!message.channel ||
			!(message.member?.roles.cache.has(tokens.roleIDs.maintainer) ?? false) ||
		...
		await webhook.send({
			content: `<@&${tokens.roleIDs.maintainer}> ` + content,
			allowedMentions: { roles: [tokens.roleIDs.maintainer] },
		});
