...
export default class LFGBaseCommand extends Subcommand {
	topics: Record<string, string> = {
		...
		games:
			"Games let you specify what you want to play. Games cannot use spaces and are case-insensitive.\nAvailable games: " +
			escapeMarkdown(QueryParser.stringify(QueryParser.games)),
		tags:
			"Tags let you filter a game to be something more specific. Tags cannot use spaces and are case-insensitive.\nAvailable tags: " +
			escapeMarkdown(QueryParser.stringify(QueryParser.tags)),
		languages:
			"You can specify what languages you can speak with the `!languages` command.",
	};

	async help(msg: Message, args: Args): Promise<void> {
		const topic = await args
			.pick({ name: "topic", type: "string" })
			.catch(() => "basics");
		if (!Object.keys(this.topics).includes(topic)) {
			await msg.reply(
				`Unknown topic: ${topic}. Available topics: ${Object.keys(this.topics).join(", ")}.`,
			);
			return;
		}
	...
	async invite(message: Message, args: Args): Promise<void> {
		...
		LFG.invite(message, players).catch(
			Logger.errorReply("invite users", message),
		);
	}
		...
		for (const player of LFG.players) {
			if (player.user == message.author.id) {
				if (player.message != null) {
					const lfgMessage = await message.channel.messages.fetch(
						player.message,
					);
