...
export default class ChannelPrecondition extends Precondition {
	run(
		...
		if (
			command != null &&
			(command.name == "refresh-rolemenu" ||
				(command.name == "agree" && message.channel.name == "rules"))
		)
			return this.ok();
		...
		if (
			command != null &&
			command.name == "maintainers" &&
			message.channel.name.startsWith("repo-")
		)
			return this.ok();

		// ephemeral commands can be used in any channel if it's an interaction
		if (command != null && command.options.ephemeral) {
			if (message instanceof ChatInputCommandInteraction) return this.ok();

			return this.error({
				message:
					"That command can be used as a slash command or go to <#394275199509594113>.",
			});
		}

		return this.error({
			message:
				"That command isn't allowed in this channel. Go to <#394275199509594113>.",
		});
	}
